...
export class TodayService {
  ...
  /** Send the digest to every admin-web member of every org (or one org). */
  async sendDigest(onlyOrgId?: string): Promise<{ sent: number; skipped: number }> {
    ...
    for (const org of orgs) {
      ...
      for (const m of members) {
        ...
        try {
          const data = await this.getToday(org.id, m.user.id, [m.role]);
          const { subject, body } = this.buildDigestText(m.user.name?.split(" ")[0] || "there", data, appUrl);
          await this.email.send(m.user.email, subject, body, undefined, org.id);
          sent++;
        } catch (e: any) {
