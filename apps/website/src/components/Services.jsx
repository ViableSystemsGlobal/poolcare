...
function Services({ showSpecialized = true }) {
  ...
  // CMS items carry copy; presentation-only keys (bg, zoom) fall back to the code defaults
  const items = (s.items || SERVICES).map((it, i) => ({ ...SERVICES[i], ...it }));
...
function ServiceCard({ n, label, title, blurb, bullets, bg, image, zoom, index, total, bind, idx }) {
  const b = bind || (() => ({}));
  return (
    <article className="app-stack__card" style={{ top: `calc(clamp(72px, 12vh, 100px) + ${index * 14}px)`, zIndex: index + 1 }}>
      <div className="app-stack__card-inner" style={{ position: 'relative', background: bg, border: 'none' }}>
        {/* zoom crops the left of the photo off-canvas — for images with busy left halves */}
        <img loading="lazy" decoding="async" src={image} alt={label} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', ...(zoom ? { transform: `scale(${zoom})`, transformOrigin: 'right center' } : {}) }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        {/* Scrim keeps the left text column readable over any photo */}
        <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(8,26,17,0.94) 0%, rgba(8,26,17,0.84) 40%, rgba(8,26,17,0.42) 66%, rgba(8,26,17,0.05) 88%)' }} />
        <div className="pad-card-lg" style={{ position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: 540, maxWidth: 680 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.16em', color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase' }}>{n} / {String(total).padStart(2, '0')}</span>
            <span style={{ height: 1, width: 32, background: 'rgba(255,255,255,0.3)' }}/>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.14em', color: 'rgba(255,255,255,0.85)', textTransform: 'uppercase', fontWeight: 500 }} {...b(`items.${idx}.label`)}>{label}</span>
          </div>
          <h3 className="display-3" style={{ margin: 0, fontSize: 'clamp(32px, 3.6vw, 56px)', whiteSpace: 'pre-line', color: '#fff' }} {...b(`items.${idx}.title`)}>{title}</h3>
          <p className="h-lead" style={{ marginTop: 24, maxWidth: '38ch', color: 'rgba(255,255,255,0.78)' }} {...b(`items.${idx}.blurb`)}>{blurb}</p>
          <ul style={{ marginTop: 32, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 14 }} {...b(`items.${idx}.bullets`)}>
            {bullets.map((bl, j) => (
              <li key={j} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 15, color: 'rgba(255,255,255,0.9)' }}>
                <span style={{ width: 22, height: 22, borderRadius: 999, background: 'rgba(255,255,255,0.16)', border: '1px solid rgba(255,255,255,0.28)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
